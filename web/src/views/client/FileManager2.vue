<template>
  <div class="file-manager">
    ...
    <div v-if="showPreviewDialog" class="custom-popup-overlay" @click.self="closePreview">
      <div class="custom-popup preview-popup modern-preview" :class="{ 'pdf-preview': isPdf }">
        ...
        <div class="preview-content">
          ...
          <div v-else-if="isAudio" class="audio-preview-modern">
            ...
            <div class="audio-player-controls">
              ...
              <div class="control-buttons">
                <button class="control-btn" @click="togglePlayPause" :disabled="!duration || duration === 0">
          ...
          <div v-else-if="isPdf" class="pdf-preview-container">
            <div v-if="pdfLoading" class="pdf-loading">
              <var-icon name="loading" :size="48" spin />
              <p>加载 PDF 文件中...</p>
            </div>
            <iframe
              v-show="!pdfLoading"
              ...
              @load="onPdfLoaded"
            ></iframe>
            <div class="pdf-toolbar" v-if="!pdfLoading">
              <button class="pdf-toolbar-btn" @click="openPdfInNewWindow" title="在新窗口打开">
                <var-icon name="open-in-new" :size="18" />
              </button>
              <button class="pdf-toolbar-btn" @click="downloadCurrent" title="下载">
                <var-icon name="download" :size="18" />
              </button>
              <span class="pdf-info">{{ pdfFileName }} ({{ formatFileSize(previewFile?.size) }})</span>
            </div>
          </div>
...
const isText = computed(() => previewFile.value?.mime_type?.startsWith('text/') || previewFile.value?.mime_type === 'application/json')
...
const openPdfInNewWindow = () => {
  if (previewFile.value) {
    const url = fileApi.preview(previewFile.value.path)
    window.open(url, '_blank')
    Snackbar.info('PDF 已在新窗口中打开')
  }
}
...
const togglePlayPause = () => {
  ...
  if (!duration.value || duration.value === 0) {
    Snackbar.warning('音频未加载完成，请稍后再试')
    return
  }
...
const updateTime = () => {
  if (audioElement.value && audioElement.value.currentTime !== undefined) {
...
const onLoadedMetadata = () => {
  if (audioElement.value && !isNaN(audioElement.value.duration)) {
    ...
    console.log('音频加载成功，时长:', duration.value, '秒')
  }
}
...
const handlePreview = async (item) => {
  ...
  if (isPdf.value) {
    ...
    pdfFileName.value = item.name
    // 添加时间戳避免缓存
    pdfUrl.value = `${fileApi.preview(item.path)}&t=${Date.now()}`
  }
